  ...
  try {
    ...
    try {
      const response = await fetch(`https://nominatim.openstreetmap.org/reverse?lat=${lat}&lon=${lng}&format=json`, {
        headers: { "User-Agent": "Darji-Tailor-App/1.0" }
      });
      if (response.ok) {
...
function TailorPrimaryCta({ label, loading = false, disabled = false, onPress }: { label: string; loading?: boolean; disabled?: boolean; onPress: () => void }) {
  const unavailable = loading || disabled;
  return (
    <View style={styles.tailorPrimaryCtaSlot}>
      <Pressable onPress={onPress} disabled={unavailable} accessibilityRole="button" accessibilityLabel={label} accessibilityState={{ disabled: unavailable }}>
        {({ pressed }) => (
          <View style={[styles.tailorPrimaryCtaSurface, unavailable && styles.tailorPrimaryCtaDisabled, pressed && !unavailable && styles.tailorPrimaryCtaPressed]}>
            {loading ? (
              <ActivityIndicator color="#7b8493" />
            ) : (
              <>
                <Text style={[styles.tailorPrimaryCtaText, unavailable && styles.tailorPrimaryCtaTextDisabled]}>{label}</Text>
                <View style={[styles.tailorPrimaryCtaArrow, unavailable && styles.tailorPrimaryCtaArrowDisabled]}>
...
function RequestCard({ request, onPress }: { request: TailoringRequest; onPress: () => void }) {
  ...
  const isInstant = request.urgency?.toUpperCase().includes("INSTANT");
  return (
    <Pressable style={[styles.requestCard, isInstant && styles.instantRequestCard]} onPress={onPress}>
...
function MeasurementVisitCard({
  ...
  return (
    <View style={[styles.measureVisitCard, visit.status === "CANCELLED" && styles.measureVisitCancelledCard]}>
      ...
      {canSubmit ? (
        <TailorPrimaryCta label="Submit Measurements" onPress={() => onSubmit?.(visit)} />
      ) : null}
    </View>
  );
}
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.measureSubmitScreenWrap}>
      <ScrollView contentContainerStyle={styles.measureSubmitScreenContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <TailorPrimaryCta label="Submit Measurements" onPress={submitVisit} disabled={!canSubmitMeasurements || saving} loading={saving && otpVerified} />
      </ScrollView>
    </KeyboardAvoidingView>
  );
}
...
function GridTile({
  ...
  return (
    <Pressable
      style={({ pressed }) => (pressed ? [styles.gridCellCard, styles.gridCellCardPressed] : styles.gridCellCard)}
      onPress={onPress}
    >
        ...
        {hasMultipleItems ? (
          <View style={styles.segmentedControlContainer}>
            {items.map((item, index) => {
              ...
              return (
                <Pressable
                  key={item.id ?? `${request.id}-chip-${index}`}
    ...
    <View style={styles.proofBlock}>
      ...
      {!readOnly ? (
        <View style={styles.proofActions}>
          <Pressable style={styles.proofButton} onPress={onCamera} disabled={loadingCamera}>
            {loadingCamera ? <ActivityIndicator color={BRAND_ORANGE} /> : <Ionicons name="camera-outline" size={17} color={BRAND_ORANGE} />}
            <Text style={styles.proofButtonText}>Take live photo</Text>
          </Pressable>
        </View>
      ) : null}
    </View>
...
function OrderDetailsScreen({
  ...
  const detailItems = acceptedRequest
    ...
    : order.items.map((item, index) => ({
        ...
        service: item.service?.name ?? "Tailoring",
        gender: "Customer",
        work: item.instructions || "Custom work",
        measurement: item.measurement?.label || (item.measurement?.imageUrl ? "Sample" : "Not added"),
  ...
  return (
    <ScrollView contentContainerStyle={styles.confirmedOrderContent} showsVerticalScrollIndicator={false}>
      <View style={styles.confirmedOrderHeader}>
        <Pressable style={styles.confirmedBackButton} onPress={() => { stopVoicePlayback(); setScreen("orders", { replace: true }); }}>
          <Ionicons name="arrow-back" size={24} color={BRAND_DEEP} />
        </Pressable>
        <View style={styles.confirmedHeaderTitleWrap}>
          <View style={styles.confirmedHeaderTitleRow}>
            <Text style={styles.confirmedHeaderTitle}>Order Details</Text>
            <View style={styles.confirmedStatusPill}>
              <Text style={styles.confirmedStatusText}>{formatStatus(displayStatus)}</Text>
            </View>
          </View>
          <Text style={styles.confirmedHeaderSubtitle}>{requestCode}</Text>
        </View>
      </View>

      <View style={styles.confirmedSummaryCard}>
        <View style={styles.confirmedSummaryTop}>
          <View style={styles.confirmedThumb}>
            {firstReferenceImage ? (
              <Image source={{ uri: firstReferenceImage.url }} style={styles.confirmedThumbImage} />
            ) : (
              <Ionicons name="shirt-outline" size={34} color={BRAND_ORANGE} />
            )}
          </View>
          <View style={styles.confirmedSummaryMain}>
            <Text style={styles.confirmedGarmentTitle} numberOfLines={1}>{primaryDetailItem?.title ?? firstItem(order)}</Text>
            <Text style={styles.confirmedGarmentMeta} numberOfLines={1}>{primaryDetailItem?.service ?? "Tailoring"}</Text>
            <Text style={styles.confirmedRequestCode}>{requestCode}</Text>
          </View>
          <View style={styles.confirmedValueBox}>
            <Text style={styles.confirmedValueLabel}>Order Value</Text>
            <Text style={styles.confirmedValueAmount}>{money(order.totalAmount)}</Text>
          </View>
        </View>
        <View style={styles.confirmedTimeline}>
          {timelineSteps.map((step, index) => (
            <View key={step.label} style={styles.confirmedTimelineItem}>
              ...
              <Text style={[styles.confirmedTimelineLabel, step.active && styles.confirmedTimelineLabelActive]} numberOfLines={1}>{step.label}</Text>
              {index < timelineSteps.length - 1 ? <View style={styles.confirmedTimelineLine} /> : null}
            </View>
          ))}
        </View>
      </View>
              ...
              {(order.status === "WORKING" || order.status === "TAILOR_STARTED") && (
                <>
                  <ProofBlock
                    ...
                    loadingCamera={uploadingProof?.stage === "STITCHED"}
                    onCamera={() => uploadProof("STITCHED")}
                    onOpen={setSelectedMedia}
  ...
  return (
    ...
    >
      ...
      <View style={styles.walletTransactionSide}>
        <Text style={[styles.walletTransactionAmount, { color: isCredit ? SUCCESS : "#b91c1c" }]}>{isCredit ? "+" : "-"}{money(transaction.amount)}</Text>
        {proofUrl ? (
          <Pressable
  ...
  return (
    <ScrollView contentContainerStyle={styles.pageContent} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.walletSummaryCard}>
        <View style={styles.walletSectionHeading}>
          <Ionicons name="calendar-outline" size={21} color={BRAND_ORANGE} />
      ...
      ) : <EmptyState icon="wallet-outline" title="No transactions yet" copy="Completed order earnings and weekly payouts will appear here." />}
...
function TransactionHistoryScreen({ wallet, onOpenOrder, showDialog }: {
  ...
  return (
    <ScrollView contentContainerStyle={styles.pageContent} showsVerticalScrollIndicator={false}>
      <Header title="Transaction History" subtitle="Every wallet credit and payout" />
      ...
      {entries.length ? <View style={styles.walletHistoryCard}>{entries.map((transaction) => <WalletTransactionCard key={transaction.id} transaction={transaction} payments={wallet.payments} onOpenOrder={onOpenOrder} showDialog={showDialog} />)}</View> : <EmptyState icon="receipt-outline" title="No transactions on this date" copy="Choose another date or clear the calendar filter." />}
    </ScrollView>
  );
}
...
export default function App() {
  ...
  async function openWalletOrder(orderId: string) {
    const order = orders.find((item) => item.id === orderId || item.request?.id === orderId || item.id === `accepted-${orderId}`);
    if (order) {
      setActiveOrder(order);
      setScreen("orderDetails");
      return;
    ...
    try {
      const loadedOrder = await api<Order>(`/orders/${orderId}`, {}, token);
      setOrders((current) => [loadedOrder, ...current.filter((item) => item.id !== loadedOrder.id)]);
      setActiveOrder(loadedOrder);
      setScreen("orderDetails");
      return;
    } catch {
      // Some tailoring earnings reference the tailoring request rather than
      // the generated order, so resolve that shape as the second source.
    }
    try {
      const request = await api<TailoringRequest>(`/tailoring-requests/${orderId}`, {}, token);
      const loadedOrder = orderFromAcceptedRequest(request);
      setOrders((current) => [loadedOrder, ...current.filter((item) => item.id !== loadedOrder.id)]);
      setActiveOrder(loadedOrder);
      setScreen("orderDetails");
    } catch (error) {
      setDialog({ title: "Order unavailable", message: error instanceof Error ? error.message : "This transaction is valid, but its order details could not be loaded.", icon: "receipt-outline" });
    }
  }
    ...
    try {
      const updated = await api<TailorProfile>("/tailors/me/measurement-capabilities", {
        ...
        body: JSON.stringify({
          ...
          visitPayout: me?.tailorProfile?.measurementPartner?.visitPayout ?? 30
        })
      }, token);
...
const styles = StyleSheet.create({
  ...
  instantRequestCard: { backgroundColor: "rgba(239, 68, 68, 0.10)", borderColor: "rgba(220, 38, 38, 0.45)", borderWidth: 1.5 },
  ...
  confirmedHeaderTitleRow: { flexDirection: "row", alignItems: "center", gap: 10, flexWrap: "wrap" },
  confirmedHeaderTitle: { color: BRAND_DEEP, fontSize: 26, lineHeight: 32, fontWeight: "900" },
  confirmedHeaderSubtitle: { color: MUTED, fontSize: 13, lineHeight: 18, fontWeight: "800", marginTop: 2 },
  confirmedStatusPill: { borderRadius: 999, backgroundColor: "#dcfce7", paddingHorizontal: 13, paddingVertical: 8 },
  confirmedStatusText: { color: SUCCESS, fontSize: 12, lineHeight: 15, fontWeight: "900" },
  confirmedSummaryCard: { borderRadius: 18, borderWidth: 1, borderColor: BORDER, backgroundColor: SURFACE, padding: 18, marginBottom: 16 },
  confirmedSummaryTop: { flexDirection: "row", alignItems: "center", gap: 14 },
  confirmedThumb: { width: 76, height: 76, borderRadius: 12, overflow: "hidden", backgroundColor: "#fff4dc", alignItems: "center", justifyContent: "center" },
  confirmedThumbImage: { width: "100%", height: "100%" },
  confirmedSummaryMain: { flex: 1, minWidth: 0 },
  confirmedGarmentTitle: { color: BRAND_DEEP, fontSize: 18, lineHeight: 24, fontWeight: "900" },
  confirmedGarmentMeta: { color: MUTED, fontSize: 13, lineHeight: 18, fontWeight: "800", marginTop: 3 },
  confirmedRequestCode: { color: "#dc2626", fontSize: 14, lineHeight: 19, fontWeight: "900", marginTop: 8 },
  confirmedValueBox: { alignItems: "flex-end", minWidth: 74 },
  confirmedValueLabel: { color: MUTED, fontSize: 12, lineHeight: 16, fontWeight: "800", textAlign: "right" },
  confirmedValueAmount: { color: BRAND_DEEP, fontSize: 24, lineHeight: 30, fontWeight: "900", marginTop: 8 },
  confirmedTimeline: { marginTop: 22, flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between" },
  confirmedTimelineItem: { flex: 1, alignItems: "center", minWidth: 0 },
  confirmedTimelineIcon: { width: 43, height: 43, borderRadius: 22, borderWidth: 1, borderColor: BORDER, backgroundColor: "#f8fafc", alignItems: "center", justifyContent: "center" },
  confirmedTimelineIconActive: { borderColor: "#fde68a", backgroundColor: "#fff8e1" },
  confirmedTimelineCheck: { position: "absolute", right: -1, bottom: -1, width: 17, height: 17, borderRadius: 9, backgroundColor: BRAND_ORANGE, alignItems: "center", justifyContent: "center", borderWidth: 2, borderColor: SURFACE },
  confirmedTimelineLabel: { color: MUTED, fontSize: 10, lineHeight: 14, fontWeight: "800", textAlign: "center", marginTop: 8 },
  ...
  confirmedVoiceRow: { minHeight: 86, borderRadius: 16, borderWidth: 1, borderColor: BORDER, backgroundColor: "#fbfdff", flexDirection: "row", alignItems: "center", gap: 13, paddingHorizontal: 14, paddingVertical: 12 },
  ...
  confirmedDetailText: { flex: 1, minWidth: 0 },
  confirmedDetailLabel: { color: MUTED, fontSize: 10, lineHeight: 13, fontWeight: "900" },
  confirmedDetailValue: { color: BRAND_DEEP, fontSize: 12, lineHeight: 16, fontWeight: "900", marginTop: 2 },
  confirmedMeasurements: { marginTop: 12, borderRadius: 14, borderWidth: 1, borderColor: "#eef2f7", overflow: "hidden" },
  ...
  measureSubmitOrderRow: { minHeight: 42, borderRadius: 12, backgroundColor: "#ffffff", borderWidth: 1, borderColor: "#efcf92", paddingHorizontal: 12, marginTop: 12, flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 10 },
  ...
  otpVerifyCard: { minHeight: 54, borderRadius: 14, borderWidth: 1, borderColor: "#efcf92", backgroundColor: "#fffaf0", flexDirection: "row", alignItems: "center", gap: 8, padding: 8, marginTop: 12 },
  otpVerifyInput: { flex: 1, height: 40, borderRadius: 10, borderWidth: 1, borderColor: "#efcf92", backgroundColor: "#ffffff", color: BRAND_DEEP, fontSize: 17, fontWeight: "900", textAlign: "center", letterSpacing: 4 },
  otpVerifyButton: { minWidth: 104, height: 40, borderRadius: 10, backgroundColor: BRAND_ORANGE, alignItems: "center", justifyContent: "center", paddingHorizontal: 10 },
  ...
  measureSubmitItemCard: { borderRadius: 14, borderWidth: 1, borderColor: "#efcf92", backgroundColor: "#fffaf0", padding: 11, marginBottom: 10 },
