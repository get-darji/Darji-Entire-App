...
function sendTailorSamplesUpload(files: { uri: string; name: string }[], token: string): Promise<unknown> {
  const form = new FormData();
  files.forEach((file) => {
    form.append("media", {
      uri: file.uri,
  ...
  return new Promise((resolve, reject) => {
    const request = new XMLHttpRequest();
    request.open("POST", `${apiUrl}/tailors/me/samples`);
    request.setRequestHeader("Authorization", `Bearer ${token}`);
...
export async function uploadTailorSamples(files: { uri: string; name: string }[], token?: string): Promise<unknown> {
  const currentToken = token ?? useAppStore.getState().token;
  if (!currentToken) throw new Error("Authentication required");

  try {
    return await sendTailorSamplesUpload(files, currentToken);
  } catch (error) {
    if (!sessionErrorPattern.test(error instanceof Error ? error.message : "")) throw error;
    const nextToken = await refreshAccessToken();
    if (!nextToken) throw error;
