...
export default function SupportCommandCenter({
  ...
  return (
    <div className={`darji-support-shell flex w-full gap-4 overflow-hidden text-white ${
      (onExit != null || isFullScreen)
        ? "fixed inset-0 z-[9999] p-3 bg-[#0a0d12] animate-slide-up-fade"
        : "h-[calc(100vh-170px)] animate-slide-up-fade"
    }`}>
      
      {/* COLUMN 1: Category Sidebar (240px) */}
      <div className="w-[240px] flex shrink-0 flex-col gap-3 rounded-[24px] border border-[#262b36] bg-[#171a21] p-4 shadow-2xl">
        <div className="px-1 py-1">
          <p className="text-[10px] font-bold uppercase tracking-[0.24em] text-[#7c8595]">Command Center</p>
        ...
        {onExit && (
          <button
            onClick={onExit}
            className="w-full flex items-center gap-2 mb-1 px-4 py-3 text-xs font-bold text-[#b5bbc7] hover:text-white transition bg-[#1d212b] border border-[#2d3440] hover:border-[#f98a04] rounded-[18px]"
          >
            <ArrowLeft size={14} />
            Exit Command Center
          </button>
        ...
        <nav className="flex flex-col gap-1 mt-2 flex-1 overflow-y-auto">
          {/* Customer Stream */}
          <div className="mb-4">
            <p className="px-3 mb-1 text-[11px] font-bold text-[#7c8595] uppercase tracking-wider">Customer Support</p>
            <button
              onClick={() => {
              ...
              className={`w-full flex items-center justify-between rounded-[18px] border px-4 py-3 text-left transition ${
                supportSubTab === "customer"
                  ? "border-[#f98a04] bg-[rgba(249,138,4,0.14)] text-white"
                  : "border-transparent bg-transparent text-[#b5bbc7] hover:bg-[#1d212b]"
              }`}
            >
              ...
              {counts.customer.chats > 0 && (
                <span className="px-2 py-0.5 text-xs font-bold bg-[#f98a04] rounded-full text-white">
          ...
          {/* Tailor Stream */}
          <div className="mb-4">
            <p className="px-3 mb-1 text-[11px] font-bold text-[#7c8595] uppercase tracking-wider">Tailor Support</p>
            <div className="flex flex-col gap-1">
              ...
              >
                ...
                {counts.tailor.chats > 0 && (
                  <span className="px-2 py-0.5 text-[11px] font-bold bg-[#f98a04] rounded-full text-white">
                    {counts.tailor.chats}
                  </span>
                )}
              </button>
          ...
          {/* Delivery Stream */}
          <div className="mb-4">
            <p className="px-3 mb-1 text-[11px] font-bold text-[#7c8595] uppercase tracking-wider">Delivery Support</p>
            <div className="flex flex-col gap-1">
              <button
                onClick={() => {
          ...
          {/* Bug Reports */}
          <div>
            <p className="px-3 mb-1 text-[11px] font-bold text-[#7c8595] uppercase tracking-wider">Bug Reports</p>
            <button
              onClick={() => {
      ...
      <div className="w-[340px] flex shrink-0 flex-col gap-3 rounded-[24px] border border-[#262b36] bg-[#171a21] shadow-2xl overflow-hidden">
        {/* Search Header */}
        <div className="p-4 border-b border-[#2d3440] flex flex-col gap-3">
          <div className="relative">
            <span className="absolute inset-y-0 left-3 flex items-center text-[#7c8595]">
              <Search size={15} />
            </span>
            <input
              type="text"
              ...
              className="w-full pl-9 pr-4 py-2 text-sm rounded-[16px] border border-[#2d3440] bg-[#1d212b] text-white focus:outline-none focus:border-[#f98a04]"
            />
          </div>
          ...
          <div className="flex gap-2 text-xs">
            <div className="flex-1">
              <label className="text-[10px] text-[#7c8595] font-bold block mb-1">Status</label>
              <select
                value={statusFilter}
                onChange={(e) => setStatusFilter(e.target.value)}
                className="w-full px-2 py-1 bg-[#1d212b] border border-[#2d3440] rounded-[10px] text-white text-[11px]"
              >
            ...
            {supportSubTab !== "bugs" && activeFilterTab === "chats" && (
              <div className="flex-1">
                <label className="text-[10px] text-[#7c8595] font-bold block mb-1">Priority</label>
                <select
        ...
        <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-2">
          {filteredList.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-48 text-[#7c8595] text-center px-4">
              ...
              return (
                <button
                  ...
                  className={`w-full flex flex-col gap-2 p-3 text-left rounded-[16px] border transition ${
                    isSelected
                      ? "border-[#f98a04] bg-[#1d212b] text-white"
                      : "border-[#262b36] bg-[#171a21] hover:bg-[#1d212b] text-[#b5bbc7]"
                  }`}
                >
                  <div className="flex justify-between items-start w-full">
                    <span className="font-semibold text-sm truncate flex-1 text-white pr-2 flex items-center gap-1.5">
                      {name}
                    ...
                    <span className="text-[10px] text-[#7c8595] shrink-0 font-medium">{timeStr}</span>
                  </div>

                  <p className="text-xs truncate w-full text-[#b5bbc7]">{subtitle}</p>
                  ...
                  <div className="flex justify-between items-center w-full mt-1">
                    ...
                    {isUnread && (
                      <span className="w-2.5 h-2.5 rounded-full bg-[#f98a04]" />
                    )}
                  </div>
                </button>
              );
      ...
      <div className="flex-1 flex flex-col rounded-[24px] border border-[#262b36] bg-[#171a21] shadow-2xl overflow-hidden relative">
        {activeId ? (
          <>
            {/* Chat Area Header */}
            <div className="px-6 py-4 border-b border-[#2d3440] flex justify-between items-center bg-[#1c202a] shrink-0">
              <div className="flex flex-col gap-1">
                ...
                <p className="text-xs text-[#7c8595] max-w-[400px] truncate">
              ...
              <div className="flex items-center gap-3">
                ...
                {otherViewer && (
                  <span className="text-xs bg-[#f98a04]/10 border border-[#f98a04]/30 text-[#f98a04] px-2 py-1 rounded-[12px] flex items-center gap-1.5 animate-pulse">
                    <User size={13} />
                ...
                {(activeType === "ticket" || activeType === "bug") && (
                  <select
                    ...
                    className="px-3 py-1.5 bg-[#1d212b] border border-[#2d3440] rounded-[12px] text-white text-xs font-semibold focus:outline-none focus:border-[#f98a04]"
                  >
                    {activeType === "ticket" ? (
                      <>
                        <option value="OPEN">Open</option>
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-[12px] text-xs font-semibold border flex items-center gap-1.5 transition ${
                      (activeTicket?.assignedTo === me.id || activeBug?.assignedTo === me.id)
                        ...
                        : "bg-[#1d212b] border-[#2d3440] hover:border-[#f98a04] text-[#b5bbc7]"
                    }`}
                  >
                    <UserCheck size={14} />
                ...
                {onExit == null && (
                  <button
                    ...
                    className="p-2 rounded-[12px] text-[#7c8595] hover:text-white hover:bg-[#1d212b] border border-transparent hover:border-[#2d3440] transition"
                  >
            ...
            <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4">
              ...
              {activeType === "bug" && activeBug && (
                <div className="p-4 bg-[#1d212b] border border-[#2d3440] rounded-[18px] text-sm text-[#b5bbc7]">
                  <p className="font-bold text-white mb-2">Bug Description:</p>
                  <p className="mb-3 whitespace-pre-wrap">{activeBug.description}</p>
                  <div className="grid grid-cols-2 gap-2 text-xs text-[#7c8595]">
                  ...
                  {activeBug.screenshot && (
                    <div className="mt-3">
                      <p className="font-bold text-white mb-1.5 text-xs">Attached Screenshot:</p>
                      <img
                        src={activeBug.screenshot}
                        alt="Screenshot"
                        className="max-w-[300px] max-h-[200px] rounded-[12px] cursor-pointer hover:opacity-85 border border-[#2d3440]"
              ...
              {(activeTicket?.messages || activeBug?.messages || activeRequest?.messages || []).map((msg, i) => {
                ...
                if (isSys) {
                  return (
                    <div key={i} className="flex justify-center my-1.5 shrink-0">
                      <span className="px-3 py-1 bg-[#1d212b] border border-[#2d3440] rounded-full text-xs text-[#7c8595] font-medium">
                        {msg.text}
                      </span>
                    </div>
                  );
                ...
                return (
                  ...
                  >
                    {/* Sender Name info */}
                    <span className="text-[10px] text-[#7c8595] px-1 font-semibold">
                    ...
                    <div
                      className={`p-4 rounded-[20px] shadow-md border ${
                        isNote
                          ? "bg-[rgba(245,158,11,0.15)] border-amber-500/30 text-amber-100 rounded-tr-none"
                          : isClient
                          ? "bg-[#1d212b] border-[#2d3440] text-white rounded-tl-none"
                          : "bg-[rgba(249,138,4,0.16)] border-[#f98a04]/30 text-white rounded-tr-none"
                      }`}
                    >
                      {isNote && (
                        ...
                        <div className="mt-3 pt-3 border-t border-white/10 flex flex-col gap-2">
                          ...
                          {msg.type === "document" && (
                            <a
                              ...
                              className="flex items-center gap-2.5 p-2.5 bg-white/5 border border-white/10 hover:bg-white/10 rounded-[12px] text-xs text-white"
                            >
                              <FileText size={16} />
                    ...
                    <div className={`flex items-center gap-1.5 text-[9px] text-[#7c8595] px-1 ${
              ...
              {typingUsers[viewerKey]?.isTyping && (
                <div className="self-start flex flex-col gap-1 max-w-[70%]">
                  ...
                  <div className="p-3 bg-[#1d212b] border border-[#2d3440] text-[#7c8595] text-xs rounded-[16px] rounded-tl-none flex items-center gap-2">
            ...
            {/* Composer Box */}
            <div className="p-4 border-t border-[#2d3440] bg-[#171a21] flex flex-col gap-3 shrink-0 relative">
              {/* Attachment summary */}
              {attachment && (
                <div className="flex items-center justify-between p-2.5 bg-[#1d212b] border border-[#2d3440] rounded-[14px]">
                  <div className="flex items-center gap-2 text-xs truncate">
                    ...
                    <span className="truncate text-white font-medium">{attachment.name}</span>
                    <span className="text-[10px] text-[#7c8595]">
                      ({(attachment.size / 1024).toFixed(1)} KB)
                    </span>
                  </div>
                  <button onClick={() => setAttachment(null)} className="text-[#7c8595] hover:text-white">
                    <X size={15} />
                  </button>
                </div>
              )}
              ...
              <div className="flex items-center justify-between">
                <div className="flex gap-3 items-center">
                  {/* File Upload button */}
                  <label className="text-[#b5bbc7] hover:text-white cursor-pointer transition">
                    <Paperclip size={18} />
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold transition border ${
                      isInternal
                        ...
                        : "bg-transparent border-[#2d3440] text-[#7c8595] hover:border-[#f98a04]"
                    }`}
                  >
                    <Lock size={12} />
                ...
                {/* Info status info */}
                <span className="text-[10px] text-[#7c8595]">
                  Type <span className="font-mono text-[#f98a04] bg-[#1d212b] px-1 py-0.5 rounded">/</span> for quick templates
                </span>
              </div>
              ...
              <div className="flex gap-3 items-end">
                <div className="flex-1 relative">
                  <textarea
                    ...
                    className={`w-full max-h-36 min-h-[44px] py-3 pl-4 pr-12 text-sm rounded-[18px] border text-white focus:outline-none resize-none ${
                      isInternal
                        ? "bg-[rgba(245,158,11,0.06)] border-amber-500/30 focus:border-amber-500"
                        : "bg-[#1d212b] border-[#2d3440] focus:border-[#f98a04]"
                    }`}
                  />
                  
                  {/* Quick canned template floating menu */}
                  {showCanned && (
                    <div className="absolute bottom-full left-0 mb-2 w-80 bg-[#1c202a] border border-[#2d3440] rounded-[18px] shadow-2xl p-2 z-50 flex flex-col gap-1 max-h-60 overflow-y-auto">
                      <div className="px-3 py-1.5 text-[10px] font-bold text-[#7c8595] uppercase tracking-wider border-b border-[#2d3440]">
                        Canned Responses
                      </div>
                      {filteredCanned.length === 0 ? (
                        <div className="p-3 text-xs text-center text-[#7c8595]">No templates match search</div>
                      ) : (
                        filteredCanned.map((c) => (
                          <button
                            key={c.trigger}
                            onClick={() => handleSelectCanned(c.text)}
                            className="w-full text-left p-2.5 rounded-[12px] hover:bg-[#1d212b] text-xs flex flex-col gap-1 text-white border border-transparent hover:border-[#2d3440]"
                          >
                            <span className="font-mono text-[#f98a04] font-bold">{c.trigger}</span>
                            <span className="truncate text-[#7c8595]">{c.text}</span>
                          </button>
                        ))
                      )}
                    </div>
                ...
                <button
                  ...
                  className="p-3 bg-[#f98a04] hover:bg-[#ffaa33] disabled:opacity-40 disabled:hover:bg-[#f98a04] text-white rounded-[18px] transition shrink-0 shadow-md"
                >
                  <Send size={18} />
                </button>
              </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-[#0f1115]">
            <div className="relative mb-6">
              <div className="w-24 h-24 rounded-[36px] bg-[#171a21] border border-[#2d3440] flex items-center justify-center shadow-2xl text-[#f98a04] animate-float-gentle">
                <Volume2 size={42} />
              </div>
              <div className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-[#f98a04] border-4 border-[#0f1115] animate-pulse" />
            </div>

            <h3 className="text-xl font-bold tracking-tight text-white mb-2">Darji Support Command Center</h3>
            <p className="text-sm text-[#7c8595] max-w-sm mb-6">
            ...
            {onExit == null && (
              <button
                onClick={() => setIsFullScreen((v) => !v)}
                className="mb-6 flex items-center gap-2 px-4 py-2 rounded-[14px] bg-[#171a21] border border-[#2d3440] hover:border-[#f98a04]/50 text-[#b5bbc7] hover:text-white text-xs font-semibold transition"
              >
            ...
            <div className="grid grid-cols-3 gap-6 max-w-xl w-full border-t border-[#2d3440]/60 pt-8">
              <div className="p-4 bg-[#171a21] rounded-[20px] border border-[#2d3440]/40">
                <p className="text-2xl font-bold text-[#f98a04]">{supportStats?.openTickets ?? 0}</p>
                <p className="text-[10px] text-[#7c8595] font-bold uppercase tracking-wider mt-1">Open Tickets</p>
              </div>
              <div className="p-4 bg-[#171a21] rounded-[20px] border border-[#2d3440]/40">
                <p className="text-2xl font-bold text-[#f98a04]">
                  {changeRequests.filter((r) => r.status === "PENDING").length}
                </p>
                <p className="text-[10px] text-[#7c8595] font-bold uppercase tracking-wider mt-1">Pending Requests</p>
              </div>
              <div className="p-4 bg-[#171a21] rounded-[20px] border border-[#2d3440]/40">
                <p className="text-2xl font-bold text-[#f98a04]">
                  {bugReports.filter((b) => b.status === "NEW").length}
                </p>
                <p className="text-[10px] text-[#7c8595] font-bold uppercase tracking-wider mt-1">Unsolved Bugs</p>
              </div>
            </div>
          </div>
        )}
      ...
      {activeId && (
        <div className="w-[320px] flex shrink-0 flex-col gap-4 rounded-[24px] border border-[#262b36] bg-[#171a21] p-5 shadow-2xl overflow-y-auto">
          {/* User profile details */}
          {currentChatUser && (
            <div className="flex flex-col items-center text-center pb-4 border-b border-[#2d3440]">
              <div className="w-16 h-16 rounded-[24px] border border-[#2d3440] bg-[#1d212b] overflow-hidden flex items-center justify-center relative shadow-md">
              ...
              <h5 className="mt-3 font-bold text-sm text-white">{currentChatUser.name || "Customer"}</h5>
              <p className="text-xs text-[#7c8595] font-semibold mt-1">
                {currentChatUser.role} • {currentChatUser.phone}
              </p>
              {currentChatUser.email && (
                <p className="text-xs text-[#7c8595] mt-0.5">{currentChatUser.email}</p>
              )}
            </div>
          )}
          ...
          {activeTicket && (
            <div className="flex flex-col gap-3">
              <h6 className="text-[10px] font-bold uppercase tracking-wider text-[#7c8595]">Ticket Information</h6>
              <div className="p-3.5 bg-[#1d212b] rounded-[18px] border border-[#2d3440] text-xs flex flex-col gap-2.5">
                <div className="flex justify-between">
                  <span className="text-[#7c8595]">Category</span>
                  <span className="text-white font-semibold">{activeTicket.category || "General"}</span>
                </div>
                {activeTicket.orderId && (
                  <div className="flex justify-between">
                    <span className="text-[#7c8595]">Order ID</span>
                    <span className="text-[#f98a04] font-semibold truncate hover:underline cursor-pointer">
                      {activeTicket.order?.orderNumber || activeTicket.orderId}
                    </span>
                  </div>
                )}
                <div className="flex justify-between">
                  <span className="text-[#7c8595]">Opened</span>
          ...
          {activeRequest && (
            <div className="flex flex-col gap-4">
              <h6 className="text-[10px] font-bold uppercase tracking-wider text-[#7c8595]">Verification Request</h6>

              {/* Diff changes display */}
              <div className="p-4 bg-[#1d212b] rounded-[20px] border border-[#2d3440] text-xs flex flex-col gap-3.5">
                <div>
                  <p className="text-[10px] text-[#7c8595] uppercase font-bold tracking-wider mb-1.5">Current Values</p>
                  <pre className="p-2.5 bg-[#171a21] border border-[#2d3440] rounded-[12px] font-mono text-[11px] text-[#7c8595] whitespace-pre-wrap overflow-x-auto">
                ...
                <div>
                  ...
                  <pre className="p-2.5 bg-[#171a21] border border-green-500/20 rounded-[12px] font-mono text-[11px] text-green-400 whitespace-pre-wrap overflow-x-auto">
                ...
                {activeRequest.documents && activeRequest.documents.length > 0 && (
                  <div>
                    <p className="text-[10px] text-[#7c8595] uppercase font-bold tracking-wider mb-1.5">Verification Documents</p>
                    <div className="flex flex-col gap-1.5">
                      {activeRequest.documents.map((doc, idx) => (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="flex items-center justify-between p-2.5 bg-[#171a21] border border-[#2d3440] hover:border-[#f98a04] rounded-[12px] text-[11px] text-[#b5bbc7] transition"
                        >
                          <span className="truncate flex items-center gap-1.5">
                            <FileText size={13} />
                            Document #{idx + 1}
                          </span>
                          <ExternalLink size={12} className="shrink-0 text-[#f98a04]" />
                        </a>
                      ))}
                    </div>
                  </div>
                ...
                {activeRequest.history && activeRequest.history.length > 0 && (
                  <div className="border-t border-[#2d3440] pt-3.5">
                    <p className="text-[10px] text-[#7c8595] uppercase font-bold tracking-wider mb-2 flex items-center gap-1.5">
                      <Clock size={12} />
                      Audit Trail History
                    </p>
                    <div className="flex flex-col gap-2.5 max-h-48 overflow-y-auto pr-1">
                      {activeRequest.history.map((h, i) => (
                        <div key={i} className="p-2.5 bg-[#171a21] border border-[#2d3440] rounded-[12px] text-[10px]">
                          <div className="flex justify-between items-center mb-1">
                            <span className={`font-semibold ${h.status === "APPROVED" ? "text-green-500" : "text-red-500"}`}>
                              {h.status}
                            </span>
                            <span className="text-[#7c8595]">
                          ...
                          {h.adminNotes && <p className="text-[#7c8595] mt-1 italic">"{h.adminNotes}"</p>}
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              {activeRequest.status === "PENDING" && (
                <div className="flex flex-col gap-2 mt-2">
                  ...
                  <button
                    ...
                    className="w-full py-3 bg-red-600/20 border border-red-500/30 hover:bg-red-600/30 text-red-400 text-xs font-bold rounded-[16px] transition flex items-center justify-center gap-1.5"
                  >
                    <X size={16} />
                    Reject Update
                  </button>
          ...
          <div className="flex flex-col gap-3 mt-auto">
            <h6 className="text-[10px] font-bold uppercase tracking-wider text-[#7c8595]">Activity Log</h6>
            <div className="p-3.5 bg-[#1d212b] rounded-[18px] border border-[#2d3440] text-[10px] text-[#7c8595] flex flex-col gap-2">
              <div className="flex items-start gap-1.5">
                <span className="w-1.5 h-1.5 bg-green-500 rounded-full mt-1 shrink-0" />
                <span>Operator connected securely</span>
              </div>
              <div className="flex items-start gap-1.5">
                <span className="w-1.5 h-1.5 bg-[#f98a04] rounded-full mt-1 shrink-0" />
