...
const outDir = path.join('C:/Users/amank/Desktop/app', 'tmp-playwright-output');
...
(async () => {
  ...
  results.mobile.faqContainsAnswer = (await mobilePage.locator('#faq').textContent())?.includes('Darji');
  ...
  await desktopPage.locator('nav').getByRole('link', { name: /How It Works/i }).click();
